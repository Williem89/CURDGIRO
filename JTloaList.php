...
$total_records = count($jt_loa_records);
$records_per_page = 30;
$total_pages = ceil($total_records / $records_per_page);
$current_page = isset($_GET['page']) ? (int) $_GET['page'] : 1;
$offset = ($current_page - 1) * $records_per_page;
$current_records = array_slice($jt_loa_records, $offset, $records_per_page);
...
function exportToExcel($records)
{
    try {
        ...
        foreach ($records as $loa) {
            $sheet->setCellValue("A$row", $loa['tanggal_jatuh_tempo']);
            $sheet->setCellValue("B$row", $loa['noloa']);
            $sheet->setCellValue("C$row", $loa['ac_number']);
            $sheet->setCellValue("D$row", $loa['namabank']);
            $sheet->setCellValue("E$row", $loa['ac_penerima']);
            $sheet->setCellValue("F$row", $loa['nama_penerima']);
            $sheet->setCellValue("G$row", $loa['bank_penerima']); // Ensure this matches your database column
            $sheet->setCellValue("H$row", $loa['PVRNo']);
            $sheet->setCellValue("I$row", $loa['keterangan']);
            $sheet->setCellValue("J$row", $loa['Nominal']);
            $row++;
        }

        // Save Excel file
        $filename = "loa_Jatuh_Tempo.xlsx";
...
<body>
    <div class="container" style="width:100%; max-width:2000px">
        <h1 class="text-center">DAFTAR LOA YANG AKAN DI CAIRKAN 7 HARI KEDEPAN</h1>
        <form method="POST">
            <div>
    ...
    <script>
        ...
        var reportData = <?php echo json_encode($jt_loa_records); ?>;
        let entity = {};
        ...
        function generatePDF() {
            ...
            doc.save(`Loa Jatuh Tempo - ${formattedDate}.pdf`);
        }
    </script>
