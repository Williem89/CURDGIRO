...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt->execute()) {
        ...
        } else if ($_POST['pembayaran'] == '1') {
            $jenis = 'Giro';
            $giro_sql = "UPDATE data_giro SET StatusGiro = 'Used' WHERE nogiro = ?";

            $girodetail_sql = "INSERT INTO detail_giro (
                nogiro,
                tanggal_giro,
                tanggal_jatuh_tempo,
                Nominal,
               ...
               // Prepare detail giro insert statement
               $girodetail_stmt = $conn->prepare($girodetail_sql);

               foreach ($_POST['payment_numbers'] as $index => $payment_number) {
                   $tanggalgiro = $_POST['rows'][$index]['due_date'];
                   $tanggalJatuhTempo = $_POST['rows'][$index]['due_date'];
                   $nominal = $_POST['rows'][$index]['outstanding'];
                   $namaPenerima = $_POST['nama_cust'];
                   $bankPenerima = $_POST['bank_cust'];
                   $acPenerima = $_POST['ac_cust'];
                   $statgiro = 'Pending Issued';
                   $keterangan = $_POST['pembayaran_atas'];
                   $createdBy = $_SESSION['username'];
   
                   $girodetail_stmt->bind_param(
                       "ssssssssss",
                       $payment_number,
                       $tanggalgiro,
                       $tanggalJatuhTempo,
                   ...
                   if (!$girodetail_stmt->execute()) {
                       echo "<script>alert('Error creating " . $jenis . " detail record: " . $girodetail_stmt->error . "');</script>";
                       break;
                   }
               }
               $auto_stmt->close();
               $girodetail_stmt->close();

        echo "<script>alert('Data berhasil disimpan!'); window.location.href='index.php';</script>";
        } else if ($_POST['pembayaran'] == '2') {
            $jenis = 'Cek';
            $cek_sql = "UPDATE data_cek SET StatusCek   = 'Used' WHERE nocek = ?";

            $cekdetail_sql = "INSERT INTO detail_cek (
                nocek,
                tanggal_cek,
                tanggal_jatuh_tempo,
                Nominal,
               ...
               // Prepare detail cek insert statement
               $cekdetail_stmt = $conn->prepare($cekdetail_sql);

               foreach ($_POST['payment_numbers'] as $index => $payment_number) {
                   $tanggalcek = $_POST['rows'][$index]['due_date'];
                   $tanggalJatuhTempo = $_POST['rows'][$index]['due_date'];
                   $nominal = $_POST['rows'][$index]['outstanding'];
                   $namaPenerima = $_POST['nama_cust'];
                   $bankPenerima = $_POST['bank_cust'];
                   $acPenerima = $_POST['ac_cust'];
                   $statcek = 'Pending Issued';
                   $keterangan = $_POST['pembayaran_atas'];
                   $createdBy = $_SESSION['username'];
   
                   $cekdetail_stmt->bind_param(
                       "ssssssssss",
                       $payment_number,
                       $tanggalcek,
                       $tanggalJatuhTempo,
                   ...
                   if (!$cekdetail_stmt->execute()) {
                       echo "<script>alert('Error creating " . $jenis . " detail record: " . $cekdetail_stmt->error . "');</script>";
                       break;
                   }
               }
               $auto_stmt->close();
               $cekdetail_stmt->close();
...
<body>
    ...
    <div class="container" style="padding:20px;margin-top:30px;margin-bottom:50px;height:auto;box-shadow: 0 4px 8px rgba(0, 0, 0, 0.3);border-radius: 10px;">
            ...
            <div class="row" style="margin-top:10px">
                ...
                <div class="row" style="margin-top:10px">
                    ...
                    <script>
                        ...
                        document.querySelector('input[name="cicilan_pertama"]').addEventListener('change', calculateTotalPokokHutang);
                        ...
                        function calculateTotalPokokHutang() {
                            ...
                            var cicilanPertamaChecked = document.querySelector('input[name="cicilan_pertama"]').checked;
                            var angsuranPerbulan = parseFloat(document.querySelector('input[name="angsuran_perbulan"]').value) || 0;
                            ...
                            // Add cicilan pertama to totalDp if checked
                            if (cicilanPertamaChecked) {
                                totalDp += parseFloat(document.querySelector('#tenor_table tbody tr:nth-child(1) input[name^="rows"][name$="[outstanding]"]').value) || 0;
                            }
                        ...
                        // Add sukubunga from row 1 of table_tenor
                        document.addEventListener('DOMContentLoaded', function() {
                            var sukubunga = parseFloat(document.querySelector('#tenor_table tbody tr:nth-child(1) input[name^="rows"][name$="[bunga]"]').value) || 0;
                            document.querySelector('input[name="sukubunga"]').value = sukubunga.toFixed(2);
                        });
                    </script>
