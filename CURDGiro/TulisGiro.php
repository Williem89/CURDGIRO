...
$fetch_stmt = $connection->prepare("SELECT nogiro, namabank, ac_number FROM data_giro WHERE statusgiro = 'Unused'");
$fetch_stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validate inputs
    if (empty($selected_giro_number) || empty($tanggal_giro) || empty($tanggal_jatuh_tempo) || empty($nama_penerima) || empty($bank_penerima) || empty($ac_penerima)) {
        echo "<script>alert('Error: All fields are required.');</script>";
    } else {
        // Prepare statement to insert into the detail_giro table
        $stmt = $connection->prepare("INSERT INTO detail_giro (nogiro, tanggal_giro, tanggal_jatuh_tempo, nama_penerima, bank_penerima, ac_penerima, created_by, created_at) 
             VALUES (?, ?, ?, ?, ?, ?, 'system', NOW())");
        ...
        if (!$stmt) {
            ...
            // Bind parameters
            $stmt->bind_param("ssssss", $selected_giro_number, $tanggal_giro, $tanggal_jatuh_tempo, $nama_penerima, $bank_penerima, $ac_penerima);

            // Execute statement
            if ($stmt->execute()) {
                // Update the status of the selected giro number to 'Used'
                $update_stmt = $connection->prepare("UPDATE data_giro SET statusgiro = 'Used' WHERE nogiro = ?");
